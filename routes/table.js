...
router.get("/export", async (req, res) => {
  ...
  try {
    ...
    let csv =
      "fokontany,total,homme,femme,l1,l2,l3,m1,m2\n";
...
router.post("/import", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    // ✅ sauvegarder anciennes données
    await client.query(
      `
      INSERT INTO fokontany_archive (
        archive_id,
        gid,
        geom,
        fokontany,
    ...
    await client.query(
      "TRUNCATE TABLE fokontany RESTART IDENTITY"
    );
...
router.get("/stats", async (req, res) => {
  ...
  try {
    ...
    const total = await pool.query(`
      SELECT SUM(total_1) FROM fokontany
    `);
    ...
    res.json({
      ...
      totalEtudiants:
        total.rows[0].sum || 0,
      dominant:
        dominant.rows[0]?.fokontany ||
        "Aucun",
      nbFokontany:
        nbFokontany.rows[0].count
    });
